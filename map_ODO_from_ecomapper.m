%
% function [] = map_ODO_depth_interpolated (mapfile, data_path_prefix, location)
...
%
function [] = map_ODO_from_ecomapper (mapfile, data_path_prefix, location)
...
%% finish figure
title([location ' EM measured ODO'])
ylabel('latitude')
xlabel('longitude')
cb = colorbar;
minDepth = min(data(:,3));
...
load('odo-cm.mat')
colormap(cm)
set(get(cb,'Title'),'String','ODO (mg/L)');
