% usage: test_read_current(filename,windowSize)
%        reads a .dvl file and produces plots, 
%        including smoothed for current speed and direction
function [] = test_read_dvl(filename, windowSize)
    %% read the data
    % import the data, first row is headers, take only data
    dvl = importdata(filename);
    ...
    if ( isfield(dvl,'data') )
        ...
        for x = 1:rows 
            ...
            U = Xvelocity + Xspeed;
            V = Yvelocity + Yspeed;
            Water_Current_Speed(x) = sqrt((V*V)+(U*U));
...
%         end
        ...
        title(strcat('Running average of Water\_Current\_Speed, window size: ',num2str(windowSize)))
        figure()
        hist(filteredSpeed)
        ...
        title(strcat('Running average of Water\_Current\_Direction, window size: ',num2str(windowSize)))
        figure()
